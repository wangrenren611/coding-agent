...
export function sanitizeObject<T>(obj: T, options: SanitizeOptions = {}): T {
    ...
    if (Array.isArray(obj)) {
        return obj.map((item) => sanitizeObject(item, { additionalKeys, maxDepth: maxDepth - 1, sanitizeNested })) as T;
    }
 ...
 * @returns 脱敏后的结果
 */
export function sanitizeToolResult<T extends ToolExecutionResultLike>(result: T): T {
    if (!result.result) {
        return result;
    }
