    ...
    for (let i = 0; i < definitions.length; i++) {
      ...
      try {
        ...
        if (defContent) {
          ...
          const { line: defLine, character: defChar } = defSourceFile.getLineAndCharacterOfPosition(def.textSpan.start);
    ...
    for (const refSymbol of references) {
      ...
      for (const ref of refSymbol.references) {
        ...
        try {
          ...
          if (refContent) {
            const refSourceFile = ts.createSourceFile(ref.fileName, refContent, ts.ScriptTarget.Latest, true);
            const { line: refLine, character: refChar } = refSourceFile.getLineAndCharacterOfPosition(ref.textSpan.start);
