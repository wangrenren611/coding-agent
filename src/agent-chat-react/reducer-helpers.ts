...
import type { AgentChatState, UIAssistantMessage, UIToolCall } from './types';
...
export function withToolCallUpdate(state: AgentChatState, options: ToolCallUpdateOptions): AgentChatState {
    ...
    if (existingLocator) {
        ...
        const locatedToolCall =
            locatedMessage?.kind === 'assistant' ? locatedMessage.toolCalls[existingLocator.toolIndex] : undefined;
