...
interface Line {
  ...
  "markup.heading.1": { fg: RGBA.fromHex("#000"), bold: true },
  "markup.list": { fg: RGBA.fromHex("#FF7B72") },
  "markup.raw": { fg: RGBA.fromHex("#A5D6FF") },
  default: { fg: RGBA.fromHex("#000") },
})
...
export const MessageList: React.FC<MessageListProps> = ({ messages, isLoading = false }) => {
  const lines = useMemo(() => {
    ...
    messages.forEach(message => {
      ...
      for (let i = 0; i < DISPLAY.messageGapLines; i++) {
        result.push({ content: '' });
      }
    });
    return result;
  ...
  if (messages.length === 0) {
    return (
      <box >
        <text fg={COLORS.textMuted}>Start chatting. Type /help for commands.</text>
      </box>
    );
  }
        ...
        {lines.map((line, idx) => (
          <box key={idx} flexDirection="row" >
            {line.spinner ? (
              ...
              <>
                {line.prefix && (
                  <text fg={line.prefixColor || COLORS.muted} >
                    {line.prefix}
                  </text>
                )}
