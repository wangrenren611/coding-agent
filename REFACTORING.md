...
## 附录 E: 安全实现优化 (2026-02-22 更新)

### E.1 路径安全实现

#### 最终实现方案

基于安全性与实用性的平衡，采用**多层防护策略**：
...
| 环境变量 | 默认值 | 说明 |
|----------|--------|------|
| `AGENT_ALLOW_ABSOLUTE_PATHS` | `true` | 是否允许绝对路径 |
| `AGENT_DISABLE_SENSITIVE_DIR_PROTECTION` | `false` | 是否禁用敏感目录保护 |
...
| 特性 | 初始实现 | 优化后实现 |
|------|----------|------------|
| 路径遍历检测 | 仅检查 `..` | 多层检测（编码、空字节、符号链接） |
| 敏感目录保护 | ❌ 无 | ✅ 内置黑名单 |
| 配置灵活性 | 硬编码 | 环境变量 + 代码配置 |
| 审计日志 | console.warn | 可配置回调 |
| 符号链接处理 | 基础 | 完善（解析前后双重检查） |

### E.3 安全与实用性的平衡

**设计决策**：

1. **默认允许绝对路径** - AI 编码助手需要访问项目外的文件（如临时文件、配置文件）
2. **敏感目录保护始终启用** - 防止访问 SSH 密钥、云凭证等敏感信息
3. **可配置性** - 企业用户可通过环境变量加强限制
