...
import type { AgentChatState, UIAssistantMessage, UIErrorMessage } from './types';
...
function ingestReasoningEvent(
    ...
    message: Extract<
        ...
        {
            type:
                | AgentMessageType.REASONING_START
                | AgentMessageType.REASONING_DELTA
                | AgentMessageType.REASONING_COMPLETE;
        }
    >
): AgentChatState {
    const msgId = resolveTextMessageId(state, {
        type:
            message.type === AgentMessageType.REASONING_START
    ...
    const nextReasoning =
        message.type === AgentMessageType.REASONING_DELTA
    ...
    const nextAssistant: UIAssistantMessage = {
        ...
        updatedAt: message.type === AgentMessageType.REASONING_COMPLETE ? message.timestamp : assistant.updatedAt,
    };
...
function ingestTextEvent(
    state: AgentChatState,
    message: Extract<
        AgentMessage,
        { type: AgentMessageType.TEXT_START | AgentMessageType.TEXT_DELTA | AgentMessageType.TEXT_COMPLETE }
    >
): AgentChatState {
...
function ingestUsageUpdate(
    ...
    const targetMsgId =
        ...
        resolveMessageId(state, undefined, 'usage', message.timestamp);
...
function ingestToolCreated(
    state: AgentChatState,
    message: Extract<AgentMessage, { type: AgentMessageType.TOOL_CALL_CREATED }>
): AgentChatState {
...
function ingestToolStream(
    state: AgentChatState,
    message: Extract<AgentMessage, { type: AgentMessageType.TOOL_CALL_STREAM }>
): AgentChatState {
...
function ingestToolResult(
    state: AgentChatState,
    message: Extract<AgentMessage, { type: AgentMessageType.TOOL_CALL_RESULT }>
): AgentChatState {
    const callId = toDisplayString(message.payload.callId);
    if (!callId) return state;
    const nextState = withToolCallUpdate(state, {
...
function ingestCodePatch(
    state: AgentChatState,
    message: Extract<AgentMessage, { type: AgentMessageType.CODE_PATCH }>
): AgentChatState {
