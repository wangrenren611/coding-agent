...
function mapTaskDataToManagedTask(task: TaskData): ManagedTask | null {
    ...
    const logicalTaskId = typeof managed.logicalTaskId === 'string' ? managed.logicalTaskId : null;
    if (!logicalTaskId) return null;

    const activeForm = typeof managed.activeForm === 'string' ? managed.activeForm : `${task.title}...`;
    ...
    const createdAt =
        typeof managed.createdAt === 'string' ? managed.createdAt : new Date(task.createdAt).toISOString();
    const updatedAt =
        typeof managed.updatedAt === 'string' ? managed.updatedAt : new Date(task.updatedAt).toISOString();
...
export async function loadTasks(sessionId?: string, memoryManager?: IMemoryManager): Promise<ManagedTask[]> {
...
export async function saveTaskEntry(
    ...
    const key = cacheKey(sessionId);
    const filePath = resolveTaskFilePath(sessionId);
    const current = taskListCache.has(key) ? taskListCache.get(key) || [] : await loadTasksFromFile(filePath);
    const merged = [...current.filter((item) => item.id !== task.id), task].sort((a, b) => compareTaskIds(a.id, b.id));
